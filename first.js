...
let user=0;
 ...
 let cmpara= document.querySelector("#comp");
  let uspara= document.querySelector("#user");
let cmpchoice=()=>{
    let opt=["rock","paper","scissors"];
    let rnd=Math.floor(Math.random()*3);
  console.log("computer clicked",opt[rnd]);
    return opt[rnd];
}
const showWinner=(userWin,userchoice,compchoice)=>{
    if(userWin){
        user++;
        uspara.innerText=user;
     msg.innerText=(`You Win!- ${userchoice} beats ${compchoice}.`);
     msg.style.color="green";
    }else{
        comp++;
         cmpara.innerText=comp;
...
let playGame=(userchoice)=>{
 ...
 if(userchoice===compchoice){
   msg.innerText=("it was draw,please try again.");
    msg.style.color="rgb(177, 28, 155)";
 }
 else{
   let userWin=true;
    if(userchoice==="rock"){
